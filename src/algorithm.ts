 ...
 */
function divideBall(graph: Graph, v: number, balls: Ball[], lastV = -1,
    ...
    for (const next of graph.adj[v]) {
        ...
        if (hasVisited[next]) {
            ball = new Ball('cycle', [next, v], graph.isJoint(v) ? [next, v] : [next])
            continue
        }
        const check: Ball | null = divideBall(graph, next, balls, v, hasVisited, hasFinished)
        if (!check) continue
        if (check.type === 'cycle' && check.vertices[0] === v) {
            ...
            if (graph.isJoint(v)) ball.joints.push(v)
        }
